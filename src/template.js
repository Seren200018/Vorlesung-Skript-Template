...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const highlightColors = ["yellow", "mint", "blue", "pink", "gray"];
  ...
  const notesStorageKey = "sheetNotesStateV1";
  ...
  runSafe("initSheetNotes", initSheetNotes);
  runSafe("initNoteExport", initNoteExport);
  runSafe("initNoteImport", initNoteImport);
  runSafe("initHighlightContextMenu", initHighlightContextMenu);
  runSafe("initNoteImport", initNoteImport);
  ...
  // Notes: sidecar per sheet with localStorage persistence and simple highlights.
  function ensureSheetMain(sheet) {
    // Keep existing structure; return sheet for compatibility.
    return sheet;
  }
